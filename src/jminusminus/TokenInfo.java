...
enum TokenKind {
	...
	DEC("--"), EQUAL("=="), GT(">"), GTE(">="), INC("++"), LAND("&&"), LT("<"), LE("<="), LNOT("!"), MINUS("-"), PLUS_ASSIGN("+="), MINUS_ASSIGN("-="), 
	MULT_ASSIGN("*="), DIV_ASSIGN("/="), REM_ASSIGN("%="), STAR("*"), DIV("/"), REM("%"), SHIFT_LEFT("<<"), SHIFT_RIGHT(">>"), 
	SHIFT_RIGHT_ASSIGN(">>="), SHIFT_RIGHT_UNSIGN(">>>"), SHIFT_RIGHT_UNSIGN_ASSIGN(">>>="), SHIFT_LEFT_ASSIGN("<<="), AND("&"), OR("|"), XOR("^"), 
	UCOM("~"), LPAREN("("), RPAREN(")"), LCURLY("{"), RCURLY("}"), LBRACK("["), RBRACK("]"),
...
class TokenInfo {
	 ...
	 * is IDENTIFIER. For tokens without a semantic text, it is simply its string
	 * representation. For example, "+=" when the token kind is PLUS_ASSIGN.
	 */
	private String image;
	...
	/**
	 * Construct a TokenInfo from its kind, the semantic text forming the token, and
	 * its line number.
	 * 
	...
	/**
	 * Construct a TokenInfo from its kind, and its line number. Its image is simply
