...
abstract class JBinaryExpression extends JExpression {
	...
	// Standard way of analyzing JBinaryExpression. Will differ in certain cases (like JPlusOp with Strings)
	public JExpression analyzeNumbers(Context context){
		...
		if (lhs.type() == Type.INT || rhs.type() == Type.INT) {
			...
			type = Type.ANY;
			JAST.compilationUnit.reportSemanticError(line(), "Invalid operand types");
		}
		return this;
	}
...
class JPlusOp extends JBinaryExpression {
	...
	/**
	 * Construct an AST node for an addition expression given its line number, and
	 ...
	 * that). Otherwise we check the types of the addition operands and compute the
	 * result type.
	 * 
	...
	/**
	 * Any string concatenation has been rewritten as a JStringConcatenationOp (in
	 * analyze()), so code generation here involves simply generating code for
	 * loading the operands onto the stack and then generating the appropriate add
	 * instruction.
	 * 
...
class JMultiplyOp extends JBinaryExpression {
	...
	/**
	 * Construct an AST for a multiplication expression given its line number, and
	 * the lhs and rhs operands.
	 * 
...
class JShiftRightUnsignOp extends JBinaryExpression {

	public JShiftRightUnsignOp(int line, JExpression lhs, JExpression rhs) {
		super(line, ">>>", lhs, rhs);
	}
...
class JIAndOp extends JBinaryExpression {
	public JIAndOp(int line, JExpression lhs, JExpression rhs) {
		super(line, "&", lhs, rhs);
	}
...
class JIOrOp extends JBinaryExpression {
	public JIOrOp(int line, JExpression lhs, JExpression rhs) {
		super(line, "|", lhs, rhs);
	}
...
class JIXorOp extends JBinaryExpression {
	public JIXorOp(int line, JExpression lhs, JExpression rhs) {
		super(line, "^", lhs, rhs);
	}
